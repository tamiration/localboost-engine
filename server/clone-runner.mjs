 ...
 * Runs as a child process: node clone-runner.mjs <url>
 * Completely isolated from Vite's module system. All Playwright calls wrapped
 * in individual try/catch so a single section failure never kills the whole run.
 */
...
async function safeShot(page, selectors, scroll = false) {
  for (const sel of selectors) {
    try {
      ...
      if (scroll) {
        try { await page.evaluate(y => window.scrollTo(0, Math.max(0, y - 80)), box.y); } catch {}
        try { await page.waitForTimeout(80); } catch {}
      }
      const buf = await page.screenshot({
        clip: { x: Math.max(0, box.x), y: Math.max(0, box.y), width: Math.round(box.width), height: Math.min(Math.round(box.height), 600) },
...
async function run(url) {
  ...
  try {
    ...
    // Navigate — use 'load' with a short timeout, fallback to best-effort
    let navigated = false;
    try {
    ...
    if (!navigated) {
      try { await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 8000 }); navigated = true; } catch {}
    }
    if (!navigated) {
      try { await page.goto(url, { waitUntil: 'commit', timeout: 5000 }); } catch {}
    }
    ...
    // ── 2. Hero ──────────────────────────────────────────────────────────────
    const heroData = await safeEval(page, () => {
      const h1 = document.querySelector('h1')?.innerText?.trim() || '';
      const sub = document.querySelector('h1 + p, h1 ~ p, h2, .hero p, [class*="hero"] p')?.innerText?.trim() || '';
      const ctaEl = document.querySelector('a[href*="tel"], .hero a, [class*="cta"] a, [class*="btn-primary"], [class*="btn--primary"]');
    ...
    data.hero_image_url = absUrl((meta?.ogImg || heroData?.heroImg) ?? '', url);

    const heroShot = await safeShot(page, ['.hero', '[class*="hero"]', '[class*="banner"]', 'main > section:first-child', 'header + section', 'header']);
    sections.push({
      id: 'hero', label: 'Hero Section',
      status: data.headline_template ? 'found' : 'missing',
      preview: data.headline_template || '—',
      screenshotBase64: heroShot,
    });

    // ── 3. Brand colors ──────────────────────────────────────────────────────
    const colorData = await safeEval(page, () => {
    ...
    const INVALID = new Set(['#ffffff', '#000000', '#fff', '#000', '']);
    ...
    // ── 4. Logo ───────────────────────────────────────────────────────────────
    const logoSrc = await safeEval(page, () =>
    ...
    // ── 5. Features ───────────────────────────────────────────────────────────
    const featuresRaw = await safeEval(page, () => {
      const sels = ['.features', '[class*="features"]', '.why-us', '[class*="why"]', '.benefits', '[class*="benefit"]'];
      for (const sel of sels) {
        ...
        if (results.length >= 2) return results.slice(0, 6);
      }
      return [];
    }, []);
    data.features = (featuresRaw ?? []).map(f => ({ icon: 'star', title: f.title, description: f.description }));
    const featShot = await safeShot(page, ['.features', '[class*="features"]', '.why-us', '[class*="benefit"]'], true);
    sections.push({
    ...
    // ── 6. Testimonials ───────────────────────────────────────────────────────
    const testimonialsRaw = await safeEval(page, () => {
      const sels = ['.testimonials', '[class*="testimonial"]', '.reviews', '[class*="review"]', 'blockquote'];
      for (const sel of sels) {
        const container = document.querySelector(sel);
        if (!container) continue;
    ...
    const testShot = await safeShot(page, ['.testimonials', '[class*="testimonial"]', '.reviews', '[class*="review"]'], true);
    sections.push({
    ...
    // ── 7. About ──────────────────────────────────────────────────────────────
    const aboutRaw = await safeEval(page, () => {
    ...
    const aboutShot = await safeShot(page, ['#about', '.about', '[class*="about-section"]', '[class*="about-us"]'], true);
    sections.push({
    ...
    // ── 8. Service area + map ─────────────────────────────────────────────────
    const serviceAreaRaw = await safeEval(page, () => {
    ...
    // ── 9. Before / After ─────────────────────────────────────────────────────
    const baRaw = await safeEval(page, () => {
    ...
    const bodyText = await safeEval(page, () => document.body?.innerText || '', '');
...
run(url)
  ...
  .catch(err => { process.stdout.write(JSON.stringify({ error: err.message || 'Clone failed' })); process.exit(1); });
